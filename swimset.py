...
class SwimSet:
    ...
    def SetBottomTimes(self, duration=120, distance=200, interval=180, repetitions=20, length=25,
                       direction=True):  # length in yards
        self.ltime = 0
        if direction:
            print("Near")
        else:
    ...
    def Loop(self):
        ...
        while self.RunningMode and (self.repetitions == 0 or reps < self.repetitions):
            ...
            if self.RunningMode:
                ...
                if restInterval < 0:
                    print(
                        'Slow poke, elapsedTime exceeded the interval!')  # should validate this on input and not allow it to happen
